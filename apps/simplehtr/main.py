...
# curl -X POST -F image=@data/analyze.png 'http://localhost:3200/simplehtr'
...
def infer(model, fnImg):
	...
	results["recognized"] = recognized[0]
	results["probability"] = probability[0]
	print('Recognized:', '"' + recognized[0] + '"')
	print('Probability:', probability[0])
	return results

@app.route("/simplehtr", methods=["POST"])
def simplehtr():
	"main function"
	data = {"success": False}
	...
	# infer text on test image
		...
		if flask.request.method == "POST":
			if flask.request.files.get("image"):
				...
				results = infer(model, FilePaths.fnInfer)
				print(results)
